...
{
    /// Here just so what we can use `nameof(Shared)`
    private static class Shared;
    
    /// Helper method to ensure "Shared" is always included as a dependency when needed, and to avoid typos in the string literal.
    ...
    [
        ...
        new() 
        {
            Name = "Select",  // TODO: figure out what to do with generic components
            Description = "Dropdown select for choosing a single value from a list of options; requires PopoverProvider in layout.", 
            Dependencies = CreateDeps(nameof(Popover)),
        },
        new() 
        {
        ...
        {
            Name = nameof(Tooltip), 
            Description = "Hover-triggered tooltip with pointer; requires PopoverProvider in layout.", 
            Dependencies = CreateDeps(nameof(Popover)),
        },
    ];
}

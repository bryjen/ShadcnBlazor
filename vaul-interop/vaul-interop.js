...
async function cleanupDrawer(drawerId) {
  ...
  try {
    if (drawerHostRef) {
      await drawerHostRef.invokeMethodAsync('RemoveComponent', entry.containerId);
    }
  } catch (error) {
    console.warn('Vaul: Error removing component from host:', error);
  }
...
async function openComponent(componentIdentifier, fragmentId, options, callbackRef) {
  ...
  if (!drawerHostRef) {
    console.error('Vaul: Host reference not initialized. Ensure DrawerHost component is rendered.');
    return null;
  }
  ...
  try {
    // Tell the host to show the component
    await drawerHostRef.invokeMethodAsync('ShowComponent', fragmentId, containerId);

    // Wait for the div to actually appear in the DOM
    await waitForElement(containerId, 1000);
...
if (typeof window !== 'undefined') {
  ...
  window.Vaul = {
    /**
     * Set the Drawer host reference for rendering components
     * Called from DrawerHost.razor during initialization
     * @param {DotNetObjectReference} hostRef - Reference to the DrawerHost component
     */
    setHostRef: function(hostRef) {
      drawerHostRef = hostRef;
    },
